...
# local => main_libsonnet_path = os.environ.get('PERF_MAIN_LIBSONNET_PATH', 'perf/jsonnet/main.libsonnet')
...
class UpdateGrafanaLastValueMappings:
    ...
    def __init__(self, main_libsonnet_path: str):
        ...
        self.elasticsearch = ElasticSearchOperations(es_host=os.environ.get('ELASTICSEARCH', ''),
    ...
    def get_last_elasticsearch_versions(self, last_es_fetch_days=LAST_ES_FETCH_DAYS):
        ...
        :param last_es_fetch_days: default fetch 7 days
        :return:
        """
        new_versions = {}
        ...
        for id in ids:
            ...
            for version, value in data['_source'].items():
                if version in display_versions and value not in new_versions.values() and len(value) < self.MAX_VERSION_LEN:
                    # Display only numbers in the Grafana panel, replacing 'rc' with '0' and 'ec' with '00'
                    new_versions[value.replace(".", "").replace("-", "").replace("rc", "0").replace("ec", "00")] = value.replace("rc", "0").replace("ec", "00")
